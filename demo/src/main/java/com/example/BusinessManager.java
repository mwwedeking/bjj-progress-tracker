...
public class BusinessManager {
    ...
    public Technique saveTechnique(Technique t) throws SQLException {
        ...
        if (t.getId() == 0) {
            long newId = provider.createTechnique(t);
            t.setId(newId);
        } else {
    ...
    public Technique getTechniqueById(long id) throws SQLException {
        return provider.readTechniqueById(id);
    }
    ...
    public List<Technique> getAllTechniques() throws SQLException {
        return provider.readAllTechniques();
    }
    ...
    //     return provider.readTechniqueCountById(id);
    // }
    ...
    //     return provider.readAllTechniqueCounts();
    // }
    ...
    //     if (r.getId() == 0) {
    //         long newId = provider.createRoll(r.getId(), r); //TODO: I don't think this is correct; should be session ID???
    //         r.setId(newId);
    //     } else {
    ...
    public boolean deleteRoll(long id) throws SQLException {
        ...
        try {
            Roll existing = provider.readRollById(id);
            if (existing != null) {
                if (existing.getSubs() != null) {
                    for (TechniqueCount tc : existing.getSubs()) {
                        if (tc != null && tc.getTechnique().getId() != 0) provider.deleteRollTechniqueLink(id, tc.getTechnique().getId());
                    }
                }
    ...
    public Roll getRollById(long id) throws SQLException {
        return provider.readRollById(id);
    }
    ...
    //     if (s.getId() == 0) {
    //         long newId = provider.createSession(s);
    //         s.setId(newId);
    //     } else {
    ...
    public boolean deleteSession(long id) throws SQLException {
        ...
        try {
            Session existing = provider.readSessionById(id);
    ...
    public Session getSessionById(long id) throws SQLException {
        return provider.readSessionById(id);
    }
    ...
    public List<Session> getAllSessions() throws SQLException {
        return provider.readAllSessions();
    }
}